...
signal selection_changed(newSel)
...
func _ready():
	...
	for i in range(count):
		...
		c.connect("clicked",self,"input_accept",[i])
		c.position = Vector2(100,-SPACING*center+i*SPACING-c.size.y/2)
		if item_position==POSITION.LEFT:
			c.position.x=-c.size.x-100
	...
	for i in range(count):
		...
		if item_placement==0:
			if item_position==POSITION.RIGHT:
				t.interpolate_property(c,"position:x",null,-c.size.x,.3,Tween.TRANS_CUBIC,Tween.EASE_OUT,.3+.1*i)
			else:
				t.interpolate_property(c,"position:x",null,0,.3,Tween.TRANS_CUBIC,Tween.EASE_OUT,.3+.1*i)
		else: #TODO: This is incorrect, ItemScroller frame is already center right
			c.position = Vector2(SCREEN_SIZE.x,0)
			var destX = -SPACING*center+i*SPACING+500
			t.interpolate_property(c,"position:x",null,destX,.3,Tween.TRANS_CUBIC,Tween.EASE_OUT,.3+.1*i)
	t.start()
...
func set_selection(sel):
	...
	emit_signal("selection_changed",sel)
